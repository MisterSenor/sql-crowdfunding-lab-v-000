...
"SELECT projects.title, (SUM(pledges.amount) - projects.funding_goal) AS extra_amount
FROM projects
JOIN pledges ON projects.id = pledges.project_id
GROUP BY projects.title HAVING extra_amount >= 0;"
end
